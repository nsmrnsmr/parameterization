/*平面パラメータ化
  ...
  g++ param_mv_lap.cpp -o param_mv_lap -std=c++17 -I /usr/local/.../include/eigen3
  ./param_mv_lap sample.off
*/
# include<iostream>
# include<cmath>
# include<cstdio>
...
//mvのための重みを計算する関数
double calc_mv_weight(point v1, point v2, point v3, point v4){
  double cos_v1, cos_v2, sin_v1, sin_v2;
  Vector3d a, b, c;
  a << v3.x-v1.x, v3.y-v1.y, v3.z-v1.z;
  b << v2.x-v1.x, v2.y-v1.y, v2.z-v1.z;
  c << v4.x-v1.x, v4.y-v1.y, v4.z-v1.z;
...
/*  SparseMatrix laplacian(v_num, v_num);
  ...
  for(int i=0; i<v_num; i++){ //点iに接続している頂点数の算出し、laplacianを求める
    ...
    //頂点数をで割って重みを求める部分をmvのための重みに変更する。
    double sum = 0; //繋がっている頂点の重みの合計
    do{
      int j = calc_v_tgt(F[h/3], h);
      //-1をmvのための重みに変更する。４点渡して求める。calc_mv_weight関数を利用。
      point v1, v2, v3, v4;
      v1 = p[i];
      ...
      double w = calc_mv_weight(v1,v2,v3,v4);
...
// 方程式を解く
  ...
  ofstream file;
  file.open("project2.off", ios::out);
  file << "OFF\n";
  file << v_num <<"  "<< f_num <<"  0\n";
  for(int i=0; i<v_num; i++){
    file << x(i) <<"  "<< y(i) <<"  "<< z[i] <<"\n";
